...
\begin{equation}
\label{eq:fo_quad}
\kappa^K_{\max}(\mbold r_q, t) = \frac{h_K}{2} \left( || \mbold u(\vec{t,r_q}) || + c(\vec{t,r_q}) \right),
\end{equation}
%
...
\begin{equation}
\mu^K_e(\mbold r_q,t) =  h_K^2 \frac{\max\left( | \resi^K(\mbold r_q,t) |, J^K[s](t) \right)}{|| s - \bar{s} ||_\infty}  
\end{equation}
\begin{equation}
\kappa^K_e(\mbold r_q,t) = Pr \, \mu^K_e(\mbold r_q,t)
\end{equation}
\end{subequations}
%
...
\begin{equation}
\label{eq:jump_CFEM}
J^K[s](t) = \max_{f\in\partial K}  \max_{\mbold r_q \in f} \jmp{\grad s(\mbold r_q,t) \cdot \vec{n}(\mbold r_q) }_f \, ,
\end{equation}
%
where $\jmp{a(\mbold r_q)}_f$ denotes the inter-element jump in $a(\mbold r)$ at quadrature point $\mbold r_q$ on face $f$ (the quadrature points $\mbold r_q$ are taken on the faces $f$ of the element $K$). With the definition given in \eqt{eq:jump_CFEM}, the jump is constant over each element $K$ of the computational domain. 
...
\begin{equation}
\mu^K_e(\mbold r,t)    = h_K^2 \frac{\max\left( | \resinew^K(\mbold r_q,t) |\,, || \mbold u(\mbold r_q,t) || J^K[P](t) \,, || \mbold u(\mbold r_q,t) c^2(\mbold r_q,t) || J^K[\rho](t) \right)}{\norm_P^\mu}    \, ,
\end{equation} 
\text{and} 
\begin{equation}
\kappa^K_e(\mbold r,t) = h_K^2 \frac{\max\left( | \resinew^K(\mbold r_q,t) |\,, || \mbold u(\mbold r_q,t) || J^K[P](t) \,, || \mbold u(\mbold r_q,t) c^2(\mbold r_q,t) || J^K[\rho](t) \right)}{\norm_P^\kappa} \, .
\end{equation}
\end{subequations}
%
...
%
%
\begin{figure}[H]\label{fig:outlet_sct3}
        \centering
        \begin{subfigure}[b]{0.495\textwidth}
                \centering
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
This section aims at illustrating the theoretical approach developed in \sect{sec:ev_source_terms} for solving $1$-D flow with source terms using the EVM. All of the results presented in this section were obtained with the RELAP-7 code developed by Idaho National Laboratory (REF). Three source terms are considered: the wall-fraction force and the wall heat source that were detailed in \sect{sec:ev_source_terms}, and gravity terms, which yields the following $1$-D Euler equations:
%
\begin{subequations}
...
\end{table}
%
The first test consists of a flow with only wall-friction force and gravity terms. Then, a wall-heat source is added to the system and different values of the heat transfer coefficient $h_t$ are investigated. All of the previous tests are done for both vapor and liquid water phases. Lastly, a $1$-D core channel component from RELAP-7 is used to model the core of a Pressurized Water Reactor (PWR) for liquid water.
%---------------------------------------------------------------------------------------------------
\subsection{Vapor flow in a straight pipe with wall-friction force and gravity terms.} \label{sec:1d-wall-fricition-vapor}
%---------------------------------------------------------------------------------------------------
The geometry consists of a $1$-D straight pipe of area $A=6.8 \times 10^{-5}$ $m^2$ and length $L=3.8$ $m$. The friction coefficient is set to $f_c = 0.1$. A subsonic inlet flow boundary condition is used by imposing the velocity $u_{inlet} = $ and the temperature $T_{inlet}=$. At the outlet, the static pressure $P_{outlet}=$ is specified. The initial conditions are taken equal to the boundary conditions. The code is run until steady-state with $CFL=$ and the geometry is discretized with an uniform mesh of $50$ cells. Steady-state profiles of the pressure, the velocity, the density and the viscosity coefficients are given in ....
%---------------------------------------------------------------------------------------------------
...
%---------------------------------------------------------------------------------------------------
The same geometry with the same temporal and spatial discretization as in \sect{sec:1d-wall-fricition-vapor} is used. The boundary conditions are the same except that the inlet velocity is set to $u_{inlet}=$. 
%---------------------------------------------------------------------------------------------------
\subsection{Vapor flow in a straight pipe with wall-friction force, gravity terms and wall-heat source.} \label{sec:1d-wall-source-vapor}
%---------------------------------------------------------------------------------------------------
%---------------------------------------------------------------------------------------------------
\subsection{Liquid water flow in a straight pipe with wall-friction force, gravity terms and wall-heat source.} \label{sec:1d-wall-source-liquid}
%---------------------------------------------------------------------------------------------------
%---------------------------------------------------------------------------------------------------
...
For boundary conditions, we impose, at the inlet, a mass inflow ($\rho u = 3359.62$ $kg/(m^2 s))$ and a specific enthalpy ($h = 1.28e6$ $J/kg$) and employ a static pressure condition at the exit ($P_s = 155$ bar) . The stiffened gas equation of state (SGEOS) is used \cite{SGEOS} with the following parameters: $P_{\infty} = 8.5 \cdot 10^8$ Pa, $q = -1151E+03$ $J/m^3$, $\gamma = 2.04$ and $C_v = 2069$ $J/(K \cdot kg)$. The steady-state is reached at around $t=150$ sec with a time step of $\Delta t = 0.5$ sec. Figures~\ref{fig:Pressure} through~\ref{fig:Viscosity} represent the results obtained using 20 mesh cells, using either the overly-dissipative first-order viscosity (FO), the higher-order entropy viscosity (EV), or the SUPG method to stabilize the numerical scheme. From these figures, it is clear that employing the first-order viscosity leads to erroneous answers, while the entropy-viscosity results are correct and look similar to those obtained with the SUPG \cite{SUPG} method. Plots obtained using a finer spatial resolution ($100$ cells) are also shown in these figures.
%
\begin{figure}[ht]
\centering
