...
%%
%\begin{equation}
%J = || \mbold u || \max \Big ( [[ \grad P \cdot \mbold{n} ]], c^2 [[\grad \rho \cdot \mbold{n}]] \Big) 
%\end{equation}
%\end{subequations}
%%
%where $\norm_P^\kappa$ is computed from \eqt{eq:norm_ent2}. The jump $J$ is a function of the jump of pressure and density gradients across the face with respect to its normal vector $\mbold{n}$. Then, the largest value over all faces is determined and used in the definition of the viscosity coefficients.
%
...
\end{align}
%
where $s_e$ was determined to be the inverse of the temperature and thus is positive. We note that the body forces do not affect the entropy residual. Using the result from \eqt{eq:ent_res}, \eqt{eq:ent_res_source_term} is recast as follows:
%
...
%                \caption{Supersonic flow inlet.}
        ...
        \end{subfigure}
        \caption{Subsonic (left) and supersonic (right) flow inlets\label{fig:inlet_sct3} with $u_\eta = \mbold u \cdot \mbold \eta$}
\end{figure}
%
%
\begin{figure}[H]
...
\end{table}
%    
\tcb{the $u$ in the table is the first component of $\mbold u$}
...
\end{eqnarray}   
\tcb{for non-isentropic flow, the above relations are no longer valid. I would use the Rankine-Hug. relation instead}
%========================================================
...
\end{figure}
%
The pressure decreases in the middle pipe because of the wall-friction forces as seen in \fig{fig:1d-vapor-friction-pressure-sct3} (the staircase effect is a plotting artifact). The friction force makes the pressure drop along the pipe which also makes the density drop. In \fig{fig:1d-vapor-friction-velocity-sct3} the velocity increases through the middle pipe to ensure conservation of the mass flux through the pipe at steady state. The density variation, \fig{fig:1d-vapor-friction-density-sct3}, is the same as the pressure one since they are related through the equation of state. The viscosity coefficients are shown in \fig{fig:1d-vapor-friction-visc-sct3}: because of the entropy production due to the wall-friction force, the viscosity coefficient displays a step profile. All of the variables shown in \fig{fig:1d-vapor-friction-sct3}, are constant in the first and third pipes since they are source term free.
%---------------------------------------------------------------------------------------------------
...
\end{figure}
%
The pressure, velocity, density and viscosity coefficients profiles show similar spatial variation as the vapor phase case described in \sect{sec:1d-wall-fricition-vapor}, but with different order of magnitude.
%---------------------------------------------------------------------------------------------------
...
%---------------------------------------------------------------------------------------------------
Once again, the same set up as in \sect{sec:1d-wall-fricition-vapor} and in \sect{sec:1d-wall-fricition-liquid} is used to simulate the behavior of liquid water and vapor phases with a wall-heat source. The wall temperature is assumed constant and set to $T_w=600$ $K$ for both phases. The wall heat transfer coefficients are also constant and are chosen so that a significant change in the temperature profile can be observed: $h_t^{liquid} = 1000$ $W/(K-m^2)$ and $h_t^{vapor} = 100$ $W/(K-m^2)$. The wall heated perimeter $P_w$ is the same for both phases since function of the geometry and set to $P_w = 0.0314156$ $m$. The steady-state profiles are shown in \fig{fig:1d-liquid-friction-heat-sct3} for liquid water and in \fig{fig:1d-vapor-friction-heat-sct3} for vapor. 
%
\begin{figure}[H]
        \centering
        \begin{subfigure}[b]{0.495\textwidth}
...
%---------------------------------------------------------------------------------------------------
\subsection{$1$-D Pressurized Water Reactor (PWR)} \label{sec:1d-pwr}
%---------------------------------------------------------------------------------------------------
...
\end{figure}
%
In \fig{fig:Pressure}, the steady-state pressure profile obtained with the SUPG method shows a small non-physical change of slope at the outlet that does not disappear under mesh refinement. This artifact is not seen when using the entropy viscosity method. \\
...
\end{equation}
The coefficient $k^*$ depends upon whether the first- or entropy-order viscosity coefficient is employed. When using the first-order viscosity, \eqt{eq:fo}, an expression for $\kappa_\infty$ is: $\kappa_\infty = \frac{L}{2}\left( u_\infty + c_\infty \right)$. By substituting this definition into \eqt{eq:cont_proof}, the expression obtained for the scaled continuity equation is
\begin{equation}
...
when using the first-order viscosity. However, when employing the definition of the entropy-viscosity coefficient given in \eqt{eq:final_def_visc_coeff}, it yields $k_\infty = u_\infty L_\infty$, and the dissipative term is well-scaled in the low Mach limit:
\begin{equation}
...
\end{figure}
%
This is shown in \fig{fig:Momentum}, where it is clear that the mass flux remains constant ($0.2953$ $kg/m^2s$) through the domain at steady-state when using either the entropy viscosity method or SUPG. When run with the first-order viscosity, the steady-state mass flux displays a $10\%$ variation over the domain because of the $\frac{1}{M_\infty}$ coefficient in the dissipative term of \eqt{eq:cont_proof2}.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
