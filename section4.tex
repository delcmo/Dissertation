...
%
\begin{align}
&\tilde{f}_k = \alpha_k A \kappa_k \grad \rho_k \\
&\tilde{h}_k = \alpha_k A \kappa_k \grad \left( \rho e \right)_k,
\end{align}
%  
where $\kappa_k$ is another positive viscosity coefficient. The entropy equation can now be written in its final form:
%
\begin{align}
&\alpha_k \rho_k A \frac{Ds_k}{Dt} + \left(\alpha_k \tilde{\mbold f}_k - \rho_k \mbold l_k \right) \cdot \grad s_k - \div \left( \alpha_k \rho_k A \grad s_k \right) = \nonumber\\
&- \alpha_k A \kappa_k \mathbf{Q}_k + (s_e)_k \alpha_k A \rho_k \mu_k \grad^s \mbold u_k : \grad \mbold u_k,
\end{align}
%
where $\mathbf{Q}_k$ is negative definite and defined as:
%
\begin{eqnarray}
...
uniqueness of the weak solution is of the form $\mbold l_k = \beta_k \grad \alpha_k $ where $\beta_k$
...
\item {The dissipative term $\mbold l_k$ requires the definition of a new viscosity
    ...
    account for the eigenvalue associated with the void fraction equation $u_I$.
...
\item {The partial derivatives $(s_e)_k$ and $(s_{\rho_k})_k$ can be computed using the
    definition provided in~\eqref{eq:second_th}, and are functions of the thermodynamic
...
\item {All of the dissipative terms, but the one in the volume fraction equation, are chosen to be proportional to the the void
    ...
    must to go to zero for consistency. On the other hand, when $\alpha_k$ goes to one or zero,
...
%===================================================================================
%-------------------------------------------------------------------------------------------------------------------------------------------------
\subsection{$1$-D straight pipe with wall-friction force, wall heat source and mass and energy exchange terms}\label{sec:1d-pipe-7-eq-sct4}
%-------------------------------------------------------------------------------------------------------------------------------------------------
