...
extension of the entropy-based viscosity method to low Mach fluid flows and to seven-equation two phase-flows.}
...
\left\{
\begin{array}{lll}
\text{Ideal Gas Equation Of State (IGEOS): } P = \left( \gamma-1 \right) \rho e\\[4pt]
...
\partial_t \left( \alpha_k \rho_k \vec{u_k} \right) + \dive \left[ \alpha_k \left( \rho_k \vec{u_k} \otimes \vec{u_k} + P_k \mathbf{I} \right) \right] = P_I \grad \alpha_k +\lambda_{rel} \left( \vec{u}_j - \vec{u}_k \right) \\
\partial_t \left( \alpha_k \rho_k E_k \right) + \dive \left[ \alpha_k \vec{u}_k \left( \rho_k E_k + P_k \right) \right] = P_I u_I \grad \alpha_k - \mu_{rel} \bar{P_I} \left( P_k-P_j \right) + \bar{u_I} \lambda_{rel} \left( \vec{u}_j - \vec{u}_k \right)
\end{array}
\right.
\end{equation}
where $Z_k = \rho_k c_k$ and $Z_j = \rho_j c_j$ are the impedance of the phase $k$ and $j$, respectively. The speed of sound is denoted by the variable $c$. The parameters $\mu_{rel}$ and $\lambda_{rel}$ are named pressure and velocity relaxation parameters, respectively, and a possible expression is given in \eqt{eq:sev_equ2}. They denote how fast energy and momentum are exchanged between phases.
\begin{equation}
\label{eq:sev_equ2}
\mu_{rel} = \frac{A_{int}}{Z_k + Z_j} \text{ and } \lambda_{rel} = \frac{\mu_{rel} Z_k Z_j}{2} \text{ with } A_{int} = A_{int, max} 6.75 \alpha_l \left( 1 - \alpha_l \right)^2
\end{equation}
...
The same set of equations can be obtained for the phase $j$ by simply substituting the subscript $k$ by $j$ and $j$ by $k$ in \eqt{eq:sev_equ}. The definitions of the interfacial variables $\left(P_I \right.$ and $ \left. \vec{u}_I \right)$, and the relaxation parameters $\left( \mu_{rel} \right.$ and $\left. \lambda_{rel} \right)$, remain the same. \\
...
\begin{eqnarray}
\label{eq:sev_equ4}
\rho_k A \left( \partial_t s_k + \vec{u}_k \cdot \grad s_k \right) = \frac{Z_j}{Z_j+Z_k} \lambda \left( u_k - u_j \right)^2 + \frac{Z_k}{Z_j+Z_k} \mu_{rel} \left( P_k-P_j \right)^2 + \nonumber \\ 
...
\item In order to validate this artificial viscosity method, it is proposed to run 1-D Sod shock tube \cite{1DShockSeven1} and also the same 1-D nozzle as in \cite{SEM}: it will be a good test to evaluate the capabilities of the entropy-based viscosity method to stabilize scheme while the two phases undergo mass, momentum and energy exchanges. The influence of the friction and gravity forces, and wall heat transfer source terms will be studied as well. We are also interested in running 2-D simulations that will validate our approach for the seven-equation model. 
%\cite{1DShockSeven2}. 
\end{itemize}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The 1-D grey non-equilibrium radiation-hydrodynamic equations:}  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions}
The objectives of this dissertation are to apply the entropy viscosity method to three different systems of equations: the multi-D Euler equations, the multi-D seven-equations model and the $1$-D grey non-equilibrium radiation-hydrodynamic equations. \\
